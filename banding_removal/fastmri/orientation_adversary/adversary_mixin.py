...
class AdversaryMixin(object):
    ...
    def initial_setup(self, args):
        ...
        if args.warmup_adversary_from > args.adversary_epoch_from:
            raise Exception(
                "warmup_adversary_from > adversary_epoch_from. Bad configuration!"
            )
    ...
    def model_setup(self, args):
        ...
        if args.number_of_adversaries > 1:
            adversary_model = AdversaryEnsemble(
                args.number_of_adversaries, Adversary, num_classes=1, args=args
            )
        else:
            adversary_model = Adversary(num_classes=1, args=args)

        self.model = AdversaryModel(
            prediction_model=self.model, adversary_model=adversary_model.cuda()
        )
    ...
    def adversary_adv_loss(self, prediction_reorien, true_label):
        ...
        if self.runinfo["at_epoch"] >= args.warmup_adversary_from:
            ...
            orientation_prediction_adv = self.model(
                prediction_reorien_adv, adversary=True
            )
            ...
            if args.reg_param > 0:
                reg = (
                    args.reg_param
                    * compute_grad2(
                        orientation_prediction_adv, prediction_reorien_adv
                    ).mean()
                )
            else:
    ...
    def additional_training_loss_terms(self, loss_dict, batch, prediction, target):
        ...
        if prediction is None:
            raise Exception(
                "AdversaryMixin only supports Trainers which output prediction from the training_loss"
            )
        ...
        orien_loss_adv, opred, accuracy, reg = self.adversary_adv_loss(
        ...
        if args.debug:
            ...
            prediction_reorien.register_hook(
                lambda g: self.grad_log(g, "adversary -> predictor")
            )
            prediction.register_hook(
                lambda g: self.grad_log(g, "predictor + adversary")
            )
            orientation_prediction_adv.register_hook(
                lambda g: self.grad_log(g, "adversary")
            )
    ...
    def training_loss_hook(self, progress, losses, logging_epoch):
        ...
        warm = (epoch >= self.args.warmup_adversary_from) and (
            epoch < self.args.adversary_epoch_from
        )
        ...
        if logging_epoch:
            ...
            logging.info(f"Running acc: {losses['average_accu_orien']:1.3f}")
