...
class VisualizationMixin(object):
    ...
    def visualize_dev(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(self.display_loader):
                ...
                target = transforms.center_crop_or_pad(
                    target, (self.args.resolution_height, self.args.resolution_width)
                )
                output = transforms.center_crop_or_pad(
                    output, (self.args.resolution_height, self.args.resolution_width)
                )

                if batch_idx == self.example_idx:
                    logging.info(
                        f"output std: {output.std()} target std {target.std()}"
                    )
                    logging.info(
                        f"output min: {output.min()} target min {target.min()}"
                    )
                    logging.info(
                        f"output max: {output.max()} target max {target.max()}"
                    )
                    logging.debug(
                        f"output (0.1, 1, 10, 90, 99, 99.9): {self.quantiles(output)}"
                    )
                    logging.debug(
                        f"target (0.1, 1, 10, 90, 99, 99.9): {self.quantiles(target)}"
                    )

                if grid_recons is None:
                    grid_recons = torch.zeros(
                        grid_size, output.shape[1], output.shape[2], output.shape[3]
                    ).to(self.device)
                ...
                for j in range(output.shape[0]):
                    ...
                    if self.args.display_ifft:
                        ...
                        ifft_abs = transforms.complex_abs(
                            transforms.ifft2(masked_kspace)
                        ).squeeze(0)
                        masked_image = transforms.root_sum_of_squares(
                            ifft_abs
                        ).unsqueeze(0)
                        masked_image = transforms.center_crop_or_pad(
                            masked_image,
                            (self.args.resolution_height, self.args.resolution_width),
                        )
                        grid_iffts[images_processed, ...] = masked_image.data[
                            j, ...
                        ].float()
            ...
            for i in range(images_processed):
                ...
                image_blocks.append(
                    ...
                    + (
                        ((grid_iffts[i] - shift) / scale,)
                        if self.args.display_ifft
                        else ()
                    )
                )

            grid_pil = image_grid.grid(
                image_blocks, losses=losses, runinfo=self.runinfo
            )
    ...
    def visualize_data_transform(self):
        ...
        with torch.no_grad():
            ...
            channels = (
                input[0, :, None, ...].transpose(1, 4).contiguous().view(req_shape)
            )
            grid = torchvision.utils.make_grid(
                channels, nrow=2, padding=2, normalize=True
            )
            grid_np = (
                grid.mul_(255)
                .add_(0.5)
