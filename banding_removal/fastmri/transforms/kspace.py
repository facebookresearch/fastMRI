...
def est_sens_maps(kspace, start, end, apodize_hori=0.07):
    ...
    mask = np.exp(
        -((np.linspace(-1, 1, width) / apodize_hori) ** 2), dtype=kspace.dtype
    )
    kspace = kspace * mask
...
class KSpaceDataTransform(object):
    ...
    def __call__(self, target_ksp, target_im, attrs, fname, slice):
        ...
        if self.args.coil_compress_coils:
            target_ksp = transforms.coil_compress(
                target_ksp, self.args.coil_compress_coils
            )
        ...
        input_ksp, mask, num_lf = transforms.apply_mask(
            target_ksp, self.mask_func, seed, offset, (acq_start, acq_end)
        )
        ...
        if self.args.compute_sensitivities:
            ...
            sens_map = est_sens_maps(
                kspace_np, start_of_center_mask, end_of_center_mask
            )
            sens_map = transforms.to_tensor(sens_map)

        if self.args.grappa_input:
            with h5py.File(
                self.args.grappa_input_path / self.partition / fname, "r"
            ) as hf:
        ...
        if self.args.grappa_target:
            with h5py.File(
                self.args.grappa_target_path / self.partition / fname, "r"
            ) as hf:
                kernel = transforms.to_tensor(hf["kernel"][slice])
                target_ksp = transforms.apply_grappa(
                    target_ksp.clone(), kernel, target_ksp, mask, sample_accel=2
                )
                target_im = transforms.root_sum_of_squares(
                    transforms.complex_abs(transforms.ifft2(target_ksp))
                )
